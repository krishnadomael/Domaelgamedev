...
Scoreboard.prototype.show = function(time,min,sec){

    var bmd, background,gameoverText, scoreText, highScoreText, newHighScoreText, startText, highscore;
    ...
    var hightime=localStorage.getItem('hightime',time);
    var highmin=localStorage.getItem('highmin');
    var highsec=localStorage.getItem('highsec');
    // var hightime=time;

    if(highmin<min && highsec <= sec){
        isNewHighScore=true;
        highscore=time;
    ...
    scoreText=this.game.add.bitmapText(0,200, 'minecraftia', 'Your '+time, 24);
    scoreText.x=this.game.width/2-(scoreText.textWidth/2);
    this.add(scoreText);

    highscore=this.game.add.bitmapText(0,250, 'minecraftia', 'Your Highest '+time, 24);
    highscore.x=this.game.width/2-(highscore.textWidth/2);
    this.add(highscore);
    ...
    if(isNewHighScore){
        newHighScoreText=this.game.add.bitmapText(0,100, 'minecraftia', 'Your High Score', 12);
